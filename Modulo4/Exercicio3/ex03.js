...
btn.onclick = function() {
    // pegando valor digitado no campo
    var usuario = campo.value;
    // chamar a função carrengando
    carregando();
    // pegar a url da api com a biblioteca axio
    axios.get(`https://api.github.com/users/${usuario}/repos`)
        .then(function(response) {
            // capturando nome do dono do repositório
...
function carregando() {
    ...
    // texto do li
    var txtli = document.createTextNode('Carregando...');
    // posicionando texto dentro do li
    li.appendChild(txtli);
    // adicionar li dentro da ul
    ul.appendChild(li);
}

function carregandoErro() {
    // limpando a lista
    ul.innerHTML = '';
    // pegando valor digitado no campo
    var usuario = campo.value;
    // verificando erro
    var erro = !usuario ? "Campo Obrigatório":"Usuário/Repositório inexistente";
    // criando item de lista
    var li = document.createElement('li');
    // criando texto de erro 
    var txt = document.createTextNode(erro);
    // adicionando texto no li
    li.appendChild(txt);
